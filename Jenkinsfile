      ...
      docker.image('ghcr.io/commaai/alpine-ssh').inside('--user=root') {
        timeout(time: 35, unit: 'MINUTES') {
          ...
          steps.each { item ->
            ...
            if (branch != "__jenkins_loop_test_tinygrad" && diffPaths && !hasPathChanged(gitDiff, diffPaths)) {
...
node {
  ...
  if (env.BRANCH_NAME != 'master' && env.BRANCH_NAME != '__jenkins_loop_test_tinygrad') {
    properties([
