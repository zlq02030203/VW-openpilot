...
void SpectraCamera::config_ife(int idx, int request_id, bool init) {
  ...
  */
  ...
  std::vector<uint32_t> patches;
  {
    ...
    if (!is_raw) {
      if (init) {
        buf_desc[0].length = build_initial_config((unsigned char*)ife_cmd.ptr + buf_desc[0].offset, sensor.get(), patches, cc.camera_num == 1);
      } else {
        buf_desc[0].length = build_update((unsigned char*)ife_cmd.ptr + buf_desc[0].offset, sensor.get(), patches, cc.camera_num == 1);
      }
    }
...
void SpectraCamera::configISP() {
  ...
  if (!is_raw) {
    ...
    memcpy(ife_vignetting_lut.ptr + ife_vignetting_lut.size, sensor->vignetting_lut.data(), ife_vignetting_lut.size);
  }
