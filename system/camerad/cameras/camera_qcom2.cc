...
public:
  ...
  CameraState(SpectraMaster *master, const CameraConfig &config) : camera(master, config, config.stream_type == VISION_STREAM_WIDE_ROAD) {};
  ~CameraState();
...
void camerad_thread() {
  ...
  for (const auto &config : {DRIVER_CAMERA_CONFIG, ROAD_CAMERA_CONFIG, WIDE_ROAD_CAMERA_CONFIG}) {
